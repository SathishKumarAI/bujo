...
## 500-feature backlog (appended 2026-06-22)

A ranked 572-feature backlog lives in [`FEATURE-BACKLOG-500.md`](./FEATURE-BACKLOG-500.md) — auto-generated by a 10-agent fan-out, deduped and scored. The top 10 buildable picks are being implemented; see the worklog.

## Backlog build + typing tracker + perf/UX (appended 2026-06-23)

**~147 new analytics & tracking features** were built from `FEATURE-BACKLOG-500.md`
across the health/journal domains — habit consistency/grades/heatmaps, gym
strength analytics (e1RM, muscle balance, recovery, stalled lifts), pickleball
form/matchup/venue stats, recovery time-reclaimed/portfolio/commitment, reading &
focus analytics, insights (mood impact, weekday splits), per-domain CSV/ICS
exports, and interactive items (count +/- steppers, floor/stretch targets,
custom-goal pace, money-saved counter). Most are read-only over existing data;
detail lives in each view under collapsible **Insights / Deep-analytics** sections.

**Typing practice** (Focus view): log sessions (minutes · WPM · accuracy · source),
a **1-hour weekday goal**, practice streak (weekday-scheduled), best/avg WPM, a WPM
trend chart, and quick links to Monkeytype / keybr / TypingClub / 10FastFingers /
TypeRacer. A "Typing practice" habit preset (60-min timer) is also available in
Trackers.

**Perf & UX:** all non-landing views are lazy-loaded (initial bundle 947KB→642KB);
the four largest views had their analytics cards extracted into components and
grouped under accessible collapsible sections so daily-use controls stay on top.
A `npm run smoke` test boots every view in headless Chrome to catch render breaks.

See `docs/UX-CARD-LAYOUT.csv` for the per-card arrangement recommendation.
