...
| Gym | `views/Gym.tsx` | 2-column dashboard. PPL split + next-day suggestion, routines (click-to-load), **structured `setRows`** with **previous-session + live-1RM hints**, PRs + 1RM, rest timer, **plate calculator (kg/lb plates)**, body weight, muscle map, wger exercise DB, **built-in training programs** (e.g. the pull-up program — week/day selector, load-into-session, day tracker). Units honour the kg/lb Settings toggle. |
