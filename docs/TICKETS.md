...
| BUJO-281c | ✅ **Done, and it corrected the ticket's own premise.** Pickleball digest deleted; the Correlations fold opened. **Insights: 0 folds, 530 → 249 lines, one screen and a bit at 1440.** But there was nothing to "dissolve": measured on the rendered page, all seven cards were already `background: rgba(0,0,0,0)`, `border-radius: 0`, borders `0/0/2px/0`, no shadow — `CARD.band` *is* "a heading and a hairline". **"16 Cards against a cap of 2" counted `<Card>` elements in the source, not raised cards on screen.** Third time this session an audit counted a prop and not a pixel — see BUJO-278 and the `help ?? subtitle` trap | ✅ |
| BUJO-283 | **The same correlation is printed twice on Insights, 300px apart.** Coach digest ends with `r=-0.83 · Less sleep tends to mean more stress`, and Patterns opens with `r=-0.83 · strong · Less sleep tends to mean more stress`. `coachDigest` picks its `insight` from the same `insights()` ranking Patterns lists in full, so the coach's closer is always a repeat of a line directly below it. Visible only now that both are open at once, which is what opening the fold was for. Fix is a product call: either the coach drops its `insight` line, or Patterns skips the one the coach used | 🔜 |
