import { Bell, Cloud, Database, Palette, User } from '@/components/icons'
import { Icon } from '@/components/Icon'
import { useState } from 'react'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '../components/ui/tabs'
import { Page } from '../components/shell/Page'
import {
  AppearanceTab, DataTab, ProfileTab, RemindersTab, SyncTab,
} from '../components/settings'

/**
 * SETTINGS · five tab panels and nothing else.
 *
 * This file was 969 lines: the tab shell, all five panels, nine cards, three
 * shared primitives and five file-reader handlers, so editing one label meant
 * opening the file that also holds the passcode flow and the CSV exporter.
 * Each panel is its own module under `components/settings/` now and this is
 * the shell — which is all a view in this app should be once its page has more
 * than one mode.
 *
 * The extraction was verbatim and verified as such: the rendered text of all
 * five tabs with every fold open was captured before and diffed after, because
 * markup that looks identical often is not (see the emergency-banner note in
 * the global rules). The only intended deltas are the two on the Data tab —
 * the duplicate summary card, and the calendar export that lived in two
 * places. Both are documented where they happened.
 */
