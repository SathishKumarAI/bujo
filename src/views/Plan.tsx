        ...
        <section className="space-y-5">
          ...
          <Card band title="Recurring tasks &amp; events" subtitle="Auto-added to each day they apply">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select value={type} onChange={(e) => setType(e.target.value as BulletType)} aria-label="Bullet type" className="rounded-control border border-line-strong bg-ink-0 px-2 py-2 text-body text-fg-1">
                <option value="task">task</option>
                <option value="event">event</option>
                <option value="note">note</option>
              </select>
              <select value={freq} onChange={(e) => setFreq(e.target.value as 'daily' | 'weekly')} aria-label="How often" className="rounded-control border border-line-strong bg-ink-0 px-2 py-2 text-body text-fg-1">
              ...
              {RULE_PRESETS.map((p) => {
                ...
                return (
                  <button
                    ...
                    className="rounded-control border border-line-strong bg-ink-0 px-2.5 py-1 text-label text-fg-1 hover:border-mauve disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:border-line-strong"
                  >
                    {p.text}
          ...
          <div>
            ...
            <Button variant="secondary" onClick={() => fileRef.current?.click()} className="press-3d mt-2 inline-flex items-center gap-1.5"><Icon as={CalendarPlus} size="sm" /> Choose .ics file</Button>
                  ...
                  {g.items.map((e) => (
                    <li key={e.id} className="flex flex-wrap items-center gap-x-2 gap-y-1 py-1.5 text-body">
                      ...
                      <span className="flex shrink-0 items-center gap-1">
                        <Button variant="secondary" size="sm" onClick={() => migrateEntry(e.id, today)} aria-label={`Move "${e.text}" to today`} className="press-3d">→ Today</Button>
                        <Button variant="secondary" size="sm" onClick={() => migrateEntry(e.id, addDays(today, 1))} aria-label={`Move "${e.text}" to tomorrow`} className="press-3d">→ Tomorrow</Button>
                        <Button variant="ghost" size="sm" onClick={() => dropEntry(e.id)} aria-label={`Drop "${e.text}"`} className="press-3d text-red hover:text-red">drop</Button>
                      </span>
                    </li>
                  ))}
                </ul>
            ...
            >
              <ul className="space-y-1.5 text-body">
                {deferred.slice(0, 8).map((t) => {
                  ...
                  return (
                    <li key={t.rootId} className="rounded-card bg-ink-2 px-2.5 py-1.5">
