...
export function Coaching() {
  ...
  return (
    <Page width="wide" className="gap-0 sm:gap-0">
      ...
      <CardGrid>
      {/* Program hero */}
      <Card band className={SPAN_2} title={<span className="inline-flex items-center gap-2"><Icon as={GraduationCap} size="md" className="text-mauve" /> 12-week program · beginner → 4.0</span>}
        ...
        {!start ? (
          ...
          <>
            ...
            <div className="h-2.5 overflow-hidden rounded-none bg-ink-2"><div className="h-full rounded-none transition-[width]" style={{ width: `${(done.length / ACADEMY_TOTAL_WEEKS) * 100}%`, background: cat('green') }} /></div>
      ...
      {/* Today's session */}
      <Card band title={<span className="inline-flex items-center gap-2"><Icon as={Target} size="md" className="text-teal" /> Today: {todaySlot.focus}</span>} subtitle={`${WEEKDAYS[todayDow]}, your scheduled focus`} help="A repeatable weekly split. Today's focus + a 45–60 min session template. Adapt freely; keep at least one rest day.">
        ...
        <div className="mt-3 grid grid-cols-4 gap-1 sm:grid-cols-7">
          {WEEKLY_TEMPLATE.map((d, i) => (
            <div key={d.day} className={`rounded-none p-1.5 text-center text-micro ${i === (todayDow + 6) % 7 ? 'bg-teal/20 text-teal' : 'bg-ink-0 text-fg-2'}`} title={d.focus}>
      ...
      <Card band title="The 12-week roadmap" subtitle="Tap a week to open it, check to mark it done" collapsible help="Each week builds on the last. The third-shot drop (wk 6–7) is the gate to 3.5; resets (wk 8) gate 4.0. Open a week for what to do, the drills, and the goal.">
      ...
      {/* Skill ladder */}
      <Card band title={<span className="inline-flex items-center gap-2"><Icon as={ListChecks} size="md" className="text-sky" /> Skill ladder · 2.0 → 4.5+</span>} subtitle="What to master at each level, in order" collapsible help="The skills that define each DUPR level. Master them in order — the soft game before the fast game.">
      ...
      {/* Drill library */}
      <Card band title={<span className="inline-flex items-center gap-2"><Icon as={Barbell} size="md" className="text-green" /> Drill library</span>} subtitle="By skill — tap a group" collapsible help="Concrete drills grouped by skill. Pick 1–2 per session; quality reps beat hours of casual play.">
      ...
      <Card band title={<span className="inline-flex items-center gap-2"><Icon as={Heartbeat} size="md" className="text-red" /> Knee rehab & prehab · ACL / MCL</span>} subtitle="Prevent + recover — with or without equipment" collapsible help="Prehab builds a court-proof knee; rehab supports recovery in phases. Filter by what gear you have. General education, not medical advice — after an injury, follow your physio.">
        <div className="mb-3 flex flex-wrap gap-1.5">
          {(['all', 'none', 'band', 'weights'] as const).map((e) => (
            <button key={e} onClick={() => setEquip(e)} className="rounded-none border px-2.5 py-1 text-label transition-colors"
      ...
      {/* Mental game */}
      <Card band title={<span className="inline-flex items-center gap-2"><Icon as={Brain} size="md" className="text-peach" /> Mental game</span>} subtitle="The mindset that wins close games" collapsible help="Pickleball is won between the ears at every level. Pick one principle to focus on this week.">
