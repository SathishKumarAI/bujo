...
export function FitnessHub({ initialTab = 'cardio' }: { initialTab?: 'cardio' | 'strength' }) {
  ...
  return (
    ...
    <div className={`mx-auto space-y-4 ${tab === 'strength' ? 'max-w-[84rem]' : 'max-w-read'}`}>
      ...
      <div className="flex w-fit gap-1 rounded-control bg-secondary p-1">
      ...
      <div className={`grid items-stretch gap-3 ${tab === 'cardio' ? 'sm:grid-cols-[auto_1fr]' : ''}`}>
        ...
        <button
          ...
          className="flex items-center gap-3 rounded-control bg-ink-2 px-4 py-3 text-left transition-colors hover:border-line-strong"
          title="Open the Strength tab on this split"
        >
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-control" style={washStyle(split.color)}>
...
function ActiveMinutesRing({ minutes, goal }: { minutes: number; goal: number }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-card bg-ink-2 px-4 py-3">
