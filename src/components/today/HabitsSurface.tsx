...
import { useCursor } from '../shell/Page'
...
import { useJustCaptured } from '../CaptureReceipt'
...
/**
 * HABITS · Today's fourth surface, and the whole of what was the Trackers page.
 *
 * It was `views/Trackers.tsx`, a `Body → Tracking` tab. Moved here because the
 * habit grid is part of the day, not a separate destination — the daily tick
 * already lived on Today (`TodayHabits`, `TodayCountHabits`) while the month
 * view of the same data sat two sections away.
 *
 * **The markup is not retyped.** The file was `git mv`d and only its wrapper,
 * its imports and its export shape changed, so a rendered-output diff can
 * prove the move lost nothing. That check is not optional here: this repo has
 * already lost eleven workout formats to a pass that retyped a module instead
 * of moving it, with every gate green.
 *
 * It owns its own `PageLayout`, **stacked**, for the same reason the Trackers
 * page was stacked: the month grid is 31 columns needing ~910px, and Today's
 * 62/38 split would give it a horizontal scrollbar that hides the last week of
 * the month — the surface's whole subject.
 *
 * A component rather than the hook this first was: a file whose only export is
 * a hook breaks fast refresh (`react-refresh/only-export-components`), and
