...
import { missesFor, penaltyFor, TIER_META, PENALTIES, type PenaltyTier } from '../lib/penalties'
import { Card } from './ui'
...
export function PenaltyCard() {
  ...
  return (
    <Card
      ...
      subtitle="You skipped something yesterday — pay the toll, warrior."
      right={
        <span className="rounded-full px-2 py-0.5 text-xs font-medium" style={{ background: cat(meta.color) + '22', color: cat(meta.color) }}>
          {meta.label}
        </span>
      }
    >
      ...
      <div className="rounded-xl border p-3" style={{ borderColor: cat(meta.color) + '55', background: cat(meta.color) + '11' }}>
        ...
        <p className="text-sm text-text">{penalty.task}</p>
      </div>
