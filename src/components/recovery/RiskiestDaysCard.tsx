...
export function RiskiestDaysCard({ weekdayPattern, peakWeekday }: { weekdayPattern: WeekdayPattern; peakWeekday: PeakWeekday }) {
  return (
    <Card enlargeable band hideInfo title={<span className="inline-flex items-center gap-2"><Icon as={CalendarX} size="md" className="text-red" /> Riskiest days</span>} subtitle={`${peakWeekday.label} is your highest-reset weekday`}>
      ...
      <p className="mt-1.5 text-label text-fg-2">Most resets land on <span className="font-medium" style={{ color: onRaised('red') }}>{peakWeekday.label}</span> · plan extra support there.</p>
    </Card>
  )
}
