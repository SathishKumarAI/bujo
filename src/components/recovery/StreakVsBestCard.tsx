...
export function StreakVsBestCard({
  ...
  return (
    <Card band hideInfo title="Current vs your best" subtitle={vsBest.isRecord ? 'You’re writing a new record right now' : `${vsBest.daysToBeat} day${vsBest.daysToBeat === 1 ? '' : 's'} to match your best`}>
      <div className="relative h-4 overflow-hidden rounded-none" style={{ background: cat('surface0') }}>
        {/* ghost = personal best (full width reference) */}
        <div className="absolute inset-0 rounded-none" style={{ background: cat('peach') + '2e' }} />
        {/* solid = current run, proportional to best */}
        <div className="relative h-full rounded-none transition-[width] duration-500"
      ...
      {comeback.isComeback && (
        <div className="mt-3 inline-flex w-full items-center gap-2 rounded-none p-2.5 text-body" style={{ background: cat('green') + '14', border: `1px solid ${cat('green')}44` }}>
      ...
      {!pace.alreadyRecord && pace.matchDate && (
        <div className="mt-3 inline-flex w-full items-center gap-2 rounded-none p-2.5 text-label" style={{ background: cat('mauve') + '12', border: `1px solid ${cat('mauve')}33` }}>
      ...
      {approachCopy && (
        <div className="mt-3 inline-flex w-full items-center gap-2 rounded-none p-2.5 text-label" style={{ background: cat(approachCopy.color) + '14', border: `1px solid ${cat(approachCopy.color)}44` }}>
