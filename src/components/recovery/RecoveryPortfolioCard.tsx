      ...
      <ul className="space-y-1.5">
        {portfolio.map((p, i) => (
          <li key={p.id} className="flex items-center gap-3 rounded-card bg-ink-2 px-3 py-2 text-body">
            <span className="grid h-6 w-6 shrink-0 place-items-center rounded-control text-caption font-medium" style={i === 0 ? washStyle('mauve') : { background: cat('surface0'), color: onRaised('subtext0') }}>{i + 1}</span>
