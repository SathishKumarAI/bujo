...
export function RepPRCard({ exercise, records, unit }: { exercise: string; records: RepPR[]; unit: string }) {
  return (
    <Card band title="Rep records" subtitle={<span>Best reps at each weight · <span className="text-mauve">{exercise}</span></span>} defer>
      <ul className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {records.slice(0, 9).map((r) => (
          <li key={r.weight} className="rounded-card bg-ink-2 px-3 py-2" title={`${r.reps} reps at ${r.weight}${unit}, set ${prettyDay(r.date)}`}>
            <p className="text-heading font-medium" style={{ color: onRaised('green') }}>{r.reps}<span className="ml-0.5 text-label font-normal text-fg-2">reps</span></p>
