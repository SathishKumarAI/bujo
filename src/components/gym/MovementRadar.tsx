...
export function MovementRadar({ data, unit }: { data: CategoryVolume[]; unit: string }) {
  ...
  return (
    <Card enlargeable band title="Movement balance" subtitle="Weekly volume by push / pull / legs / core" defer>
      {total === 0 ? (
        ...
        <>
          ...
          <div className="mt-1 flex flex-wrap justify-center gap-x-3 gap-y-1 text-caption text-fg-2">
            {data.map((c) => (
              <span key={c.category}>{c.category} <span className="text-fg-1">{c.volume.toLocaleString()}{unit}</span></span>
            ))}
          </div>
        </>
      )}
