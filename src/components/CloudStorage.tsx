      ...
      <div className="rounded-card bg-ink-2 p-3">
        ...
        {!isSupported() ? (
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            {s.storageMode === 'folder' && <Button variant="ghost" onClick={() => setSettings({ storageMode: 'local', folderName: undefined })} className="press-3d text-red hover:text-red">Switch to local</Button>}
          </div>
        )}
      </div>

      {/* GitHub gist */}
      <div className="mt-3 rounded-card bg-ink-2 p-3">
