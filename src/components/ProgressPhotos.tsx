...
export function ProgressPhotos() {
  ...
  return (
    <Card band
      ...
      right={
        photos.length >= 2 && (
          <Button variant="secondary" onClick={() => setCompare((c) => !c)} className="press-3d inline-flex items-center gap-1.5">
            <Icon as={Columns} size="sm" /> {compare ? 'Gallery' : 'Compare'}
          </Button>
        )
      }
    >
      <Button variant="secondary" onClick={() => ref.current?.click()} className="press-3d mb-3 flex w-full items-center justify-center gap-2">
